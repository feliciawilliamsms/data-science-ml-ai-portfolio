...
output_file="${output_dir}/AM_unpaid_claims_2024-01-01_2024-12-31_2025-01-09.csv"

# Initialize a flag to check if the header is added
header_added=false

# Loop through all files in the base directory that match the pattern
for csv_file in "$base_dir"/*/filtered_cleaned/*_merged_BNR_MPI_2024-01-01_2024-12-31_2025-01-09.nodupes.filtered_cleaned.csv.bz2.unpaid.csv; do 
    # Check if the file exists
    if [ -f "$csv_file" ]; then
        # Extract the store name from the directory structure
        ...
        echo "Warning: No files matching the patterns were found in $csv_file."
    fi
done
